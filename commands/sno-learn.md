---
description: "Understand the problem. Launch parallel research agents for domain analysis, data modeling, and codebase scouting. Then interview for gaps. Produce a spec in .sno/spec.md."
argument-hint: "[--auto]"
tools:
  read: true
  bash: true
...
<objective>
Run the sno learn phase: deep understanding before writing a single line of spec.
</objective>
...
Launching 5 agents: sno-prior-art-researcher, sno-domain-researcher, sno-data-modeler, sno-codebase-scout, sno-service-layer-analyst...

Invoke these five agents **in parallel** using the task tool. Call the task tool multiple times in a SINGLE response to run these agents concurrently. Do NOT wait for one to finish before starting the next.

1. **sno-prior-art-researcher** — How similar problems are solved in practice: established patterns, reference architectures, domain-specific gotchas, industry standards.
2. **sno-domain-researcher** — DDD analysis: bounded contexts, aggregates, entities, ubiquitous language, domain events.
3. **sno-data-modeler** — Entity identification, relationship mapping, 5NF normalization analysis.
4. **sno-codebase-scout** — Explores existing code for patterns, conventions, relevant modules, risks.
5. **sno-service-layer-analyst** — API boundaries, orchestration, transaction scoping, cross-cutting concerns.
...
1. Read all five research outputs using the read tool.
2. Collect ALL open questions from every agent.
3. Invoke the **sno-requirements-interviewer** agent using the task tool to synthesize and prioritize the questions.
...
**Before writing, re-read every research output and every answer using the read tool:**
