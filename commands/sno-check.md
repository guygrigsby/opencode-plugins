...
<objective>
Run the sno check phase: verify the work matches the spec, run tests, review changes, check acceptance criteria.
</objective>
...
1. Read `.sno/spec.md` and `.sno/plan.md` using the read tool. If they don't exist, tell the user to run the appropriate earlier phase first.

2. **Check acceptance criteria and review code in parallel.** Invoke the following agents simultaneously using the task tool. Call the task tool multiple times in a SINGLE response to run these agents concurrently. Do NOT wait for one to finish before starting the next.

   Launching agents: sno-pr-reviewer, sno-gemini-reviewer, verification agents, test coverage agent, README check agent...
   ...
   **sno-pr-reviewer** — runs concurrently with all other agents. Invoke using the task tool. It reviews the full diff against the base branch for code quality, security, performance, consistency, and maintainability. It returns a structured review with critical issues, warnings, nits, and a verdict (APPROVE / REQUEST CHANGES / COMMENT).

   **sno-gemini-reviewer** — runs concurrently with sno-pr-reviewer and all other agents. Invoke using the task tool. If the sno-gemini-reviewer agent fails (e.g., authentication error), continue with sno-pr-reviewer results only. Note the Gemini reviewer was skipped.
   ...
   **README check agent** — also in parallel:
   ...
   If there are only 1-2 criteria, check them directly instead of spawning agents — but always invoke the sno-pr-reviewer and sno-gemini-reviewer regardless.

3. **Collect results and update README.** Once all agents return:
   - Collect pass/fail results from verification agents.
   - Collect the PR review verdict and any critical issues or warnings from sno-pr-reviewer.
   - If sno-gemini-reviewer completed, collect its findings alongside the PR review. If it was skipped due to failure, note this in the report.
...
5. **Report results** to the user:
   ...
   - If a Gemini review was run, include its findings in the report.
...
7. If something fails (criteria or PR review critical issues), **auto-diagnose**:
   - For each failing criterion **and** each PR review critical issue, invoke a **debug agent per failure in parallel** using the task tool. Call the task tool multiple times in a SINGLE response to run these agents concurrently. Each gets:
...
- Run all checks (including PR review and Gemini review) and update the README without pausing.
