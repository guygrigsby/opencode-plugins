...
<objective>
Run the sno plan phase: turn the spec into an actionable task list with explicit dependencies so build can parallelize.
</objective>
...
1. Read `.sno/spec.md` using the read tool. If it doesn't exist, tell the user to run `/sno-learn` first.

2. **Spawn parallel plan agents.** Launching 3 agents: sno-planner, sno-ux-reviewer, sno-antipattern-detector...

   Invoke these three agents **in parallel** using the task tool. Call the task tool multiple times in a SINGLE response to run these agents concurrently. Do NOT wait for one to finish before starting the next.

   1. **sno-planner** — Analyzes the spec, research outputs, and codebase. Produces a draft plan with dependency-tracked tasks and a list of open questions/ambiguities.
   2. **sno-ux-reviewer** — Reviews user experience: interaction flows, error UX, CLI ergonomics, UI patterns, developer experience. Adapts to the interface type (CLI, TUI, GUI, API, library).
   3. **sno-antipattern-detector** — Identifies antipatterns, gotchas, and common mistakes specific to the tech stack and domain. Checks dependencies for known issues.

   Give each agent the user's description and the paths to the spec and research outputs. The spec already contains service layer analysis from the learn phase — the sno-planner should use it.

   As each agent completes, emit: "[agent-name] complete. N remaining."
...
4. **Incorporate agent findings into the draft plan.** Take the sno-planner's draft plan and enrich it with:
   - sno-ux-reviewer's must-have recommendations (error UX, interaction flows, accessibility)
   - sno-antipattern-detector's mitigations (add guardrails to relevant tasks, reorder if needed)
...
5. **Invoke the sno-critical-reviewer agent** using the task tool on the assembled draft plan. Pass it the draft plan AND the outputs from the sno-ux-reviewer and sno-antipattern-detector. It performs an adversarial review checking for:
   - Spec coverage gaps
